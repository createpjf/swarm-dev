---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Cleo deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Cleo cron scheduling for periodic checks, or version status checks on a machine running Cleo (laptop, workstation, Pi, VPS).
---
...
Assess and harden the host running Cleo, then align it to a user-defined risk tolerance without breaking access. Use Cleo security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Cleo changes the host firewall, SSH, or OS updates; it does not.
...
5. Cleo gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Cleo can access sensitive data.
...
### 2) Run Cleo security audits (read-only)

As part of the default read-only checks, run `Cleo security audit --deep`. Only offer alternatives if the user requests them:

1. `Cleo security audit` (faster, non-probing)
2. `Cleo security audit --json` (structured output)

Offer to apply Cleo safe defaults (numbered):

1. `Cleo security audit --fix`

Be explicit that `--fix` only tightens Cleo defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Cleo version/update status (read-only)

As part of the default read-only checks, run `Cleo update status`.
...
- Credential hygiene notes (location of Cleo creds, prefer disk encryption)
...
After Cleo install or first hardening pass, run at least one baseline audit and version check:

- `Cleo security audit`
- `Cleo security audit --deep`
- `Cleo update status`

Ongoing monitoring is recommended. Use the Cleo cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `Cleo cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `Cleo update status`
...
Before creating, `Cleo cron list` and match on exact `name`. If found, `Cleo cron edit <id> ...`.
If not found, `Cleo cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `Cleo update status` (preferred for source checkouts and channels)
2. `npm view Cleo version` (published npm version)

## Cleo command accuracy

Use only supported commands and flags:

- `Cleo security audit [--deep] [--fix] [--json]`
- `Cleo status` / `Cleo status --deep`
- `Cleo health --json`
- `Cleo update status`
- `Cleo cron add|list|runs|run`

Do not invent CLI flags or imply Cleo enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Cleo compaction:
